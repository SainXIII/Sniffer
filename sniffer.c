...
int main( int argc, char *argv[] )
{
	...
	/* Stringa di errore PCAP_ERRBUF_SIZE definito in pcap.h */
	char errbuf[ PCAP_ERRBUF_SIZE ];
	
	/* Espressione di filtro compilata */
	...
	/* Numero di pacchetti da sniffare */
	int num_packets = 0;
	...
	/* Fine gestione opzioni */
	
	/* Nel caso in cui il paramentro -i non sia specificato */
	if( !dev )
	{
		/* Tento di aprire il device di default */
		dev = pcap_lookupdev( errbuf );
		
		/* Se non  stato possibile aprire il device di default */
		if( !dev )
		{
	...
	/* Creo il riferimento alla sessione */
	handle = pcap_open_live( dev, BUFSIZ, 1, 1000, errbuf );
    
    
	/* Controllo che il riferimento sia stato creato con successo */
	if( !handle )
	{
	...
	/* Nel caso in cui  stato specificato il parametro -f */
	if( filter_exp )
	{	
		...
		/* Applico il filtro alla sessione */	
...
void got_packet( u_char *args, const struct pcap_pkthdr *header, const u_char *packet )
{
    ...
    /* Converto packet in una struttura di tipo ethernet_header */
	ethernet_header = ( struct ether_header * ) packet;
	
    /* Stampa il contenuto del pacchetto ethernet */
    write_ethernet( ethernet_header );

	/* Separo l'header ip dall'header ethernet */
	ip_header = ( struct ip * )( packet + ETHER_HDR_LEN );
    
    /* Stampa il contenuto del pacchetto IP */
    write_ip( ip_header );
	...
	/* Separo l'header del pacchetto TCP dal restante del pacchetto */
    tcp_header = ( struct tcphdr * )( packet + ETHER_HDR_LEN + ( ( ip_header -> ip_hl * 32 ) / 8 ) );

    /* Stampo il contenuto del pacchetto TCP */
    write_tcp( tcp_header );

        
	/*Inizio stampa contenuto pacchetto */
	...
	// 160 grandezza minima del pacchetto ip senza data
}

void write_ethernet( const struct ether_header *ethernet_header )
{
    /* Inizio descrizione pacchetto ethernet */
	printf("[ETHERNET]");
	
	/* Stampo il mac di destinazione */
    ...
    /* Stampo il tipo di pacchetto ethernet */
    printf("[ether_type: ");
...
{
    /* Se il la versione IP non  4 il pacchetto non viene descritto */
    if( ip_header -> ip_v == 6 )
    {
...
void write_tcp( const struct tcphdr *tcp_header )
{
    ...
    // Numero di riscontro, ha significato solo se il flag ACK  settato a 1, e conferma la ricezione di una parte del flusso di dati nella direzione opposta, indicando il valore del prossimo Sequence number che l'host mittente del segmento TCP si aspetta di ricevere.
